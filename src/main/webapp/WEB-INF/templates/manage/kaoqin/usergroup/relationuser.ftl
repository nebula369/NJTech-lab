...
<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/kaoqin/usergroup/getUserListForPage",
            postData: {usergroupId: ${usergroupId},searchKey:$("#searchKey").val()},
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','用户姓名','帐号', '性别', '手机号码', '操作'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
        ...
        $("#btnDel").on("click", function () {
            layer.confirm("确定删除选中的角色用户?", function (index) {
                layer.close(index);
                ...
                {
                    layer.msg("请选择要删除的角色用户", {icon:2});
                    return;
                }
                jQuery.ajax({
                    type: "POST",
                    data: {"ids":teacherIds.join(","),"usergroupId":${usergroupId}},
                    dataType: "text",
                    url: "/manage/kaoqin/usergroup/DoUsergroupAndUserDel",
    ...
    function initDataList()
    {
        ...
        var postJson = {usergroupId: ${usergroupId}, searchKey:searchKey};
        //传入查询条件参数
    ...
    function del(id, name)
    {
        layer.confirm("确定移除用户“"+name+"”？", function (index) {
            ...
            jQuery.ajax({
                type: "POST",
                data: {"ids":id,"usergroupId":${usergroupId}},
                dataType: "text",
                url: "/manage/kaoqin/usergroup/DoUsergroupAndUserDel",
                success: function (result) {
                    layer.msg("删除成功", {icon: 1, time: 1000}, function () {
    ...
    function setSelectedUser(selectUsers)
    {
        ...
        jQuery.ajax({
            type: "POST",
            data: {"ids":selectRoleUserIds.join(","),"usergroupId":${usergroupId}},
            dataType: "text",
            url: "/manage/kaoqin/usergroup/DoUsergroupAndUserAdd",
            success: function (result) {
