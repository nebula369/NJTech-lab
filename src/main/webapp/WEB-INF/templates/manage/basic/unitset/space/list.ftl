        ...
        <div class="main-content-inner">
            <div class="breadcrumbs ace-save-state" id="breadcrumbs">
                <ul class="breadcrumb">
                    ...
                    <li>
                        <a href="/manage/basic/space">场地类型</a>
                    </li>
                    <li class="active">场地类型“${spaceTypeName}”下的场地列表</li>
                </ul>
            </div>
                ...
                <div class="row">
                    <div class="col-xs-12" style="padding:0px;">
                        <div style="margin-bottom: 5px;">
                            <button id="btnAdd" style="float:left; margin-top: 5px" class="btn btn-success btn-mini radius-4"><i class="ace-icon fa fa-plus"></i>新增场地</button>
                            <label style="padding-top: 5px;" class="col-sm-1 no-padding-right" for="year">场地名称</label>
                            <input type="text" id="searchKey" style="width: 250px;" placeholder="请输入场地名称/地址" class="input-sm" />
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/basic/space/getSpaceListForPage",
            postData: {typeId: ${id},searchKey:$("#searchKey").val()},
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','场地名称', '场地地址','容量','说明','关联班级', '排序号', '操作'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
                ...
                {name: 'address', index: 'address', align: 'left',sortable:true, resize:false},
                {name: 'capacity', index: 'capacity', align: 'left',sortable:true, resize:false},
        ...
        $("#btnAdd").on("click", function () {
            layer_show("新建场地", "/manage/basic/space/spaceAdd?id=${id}", 600, 430);
    ...
    function modify(id)
    {
        layer_show("编辑场地信息", "/manage/basic/space/spaceEdit?id=" + id, 600, 430);
        event.stopPropagation();
    }
    ...
    function initDataList()
    {
        ...
        var postJson = {typeId: ${id},searchKey:searchKey};
        //传入查询条件参数
    ...
    {
        layer.confirm("确定删除场地“"+name+"”？", function (index) {
            layer.close(index);
            jQuery.ajax({
                type: "POST",
                data: {"id":id},
                dataType: "text",
                url: "/manage/basic/space/spaceDoDel",
