                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <div style="margin-bottom: 5px;">
                                                <button id="btnAdd" style="float:left; margin-top: 5px" class="btn btn-success btn-mini radius-4"><i class="ace-icon fa fa-plus"></i>选择角色用户</button>
...
<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/basic/roleAuth/getRoleUserListForPage",
            postData: {roleId: ${roleId},searchKey:$("#searchKey").val()},
        ...
        $("#btnDel").on("click", function () {
            layer.confirm("确定删除选中的角色用户?", function (index) {
                ...
                jQuery.ajax({
                    type: "POST",
                    data: {"ids":teacherIds.join(","),"roleId":${roleId}},
                    dataType: "text",
                    url: "/manage/basic/roleAuth/doBatchDel",
    ...
    function initDataList()
    {
        ...
        var postJson = {roleId: ${roleId}, searchKey:searchKey};
        //传入查询条件参数
    ...
    {
        layer.confirm("确定移除角色用户“"+name+"”？", function (index) {
            layer.close(index);
            jQuery.ajax({
                type: "POST",
                data: {"id":id,"roleId":${roleId}},
                dataType: "text",
                url: "/manage/basic/roleAuth/doRoleUserDel",
                success: function (result) {
                    layer.msg("删除成功", {icon: 1, time: 1000}, function () {
                        reloadData();
                    });
    ...
    function setSelectedUser(selectUsers)
    {
        ...
        jQuery.ajax({
            type: "POST",
            data: {"ids":selectRoleUserIds.join(","),"roleId":${roleId}},
            dataType: "text",
            url: "/manage/basic/roleAuth/roleUserDoAdd",
            success: function (result) {
