...
<div class="main-container ace-save-state" id="main-container">
    ...
    <!-- 加载左侧菜单 -->
    <@leftMenu indexUrl="/manage/profile/index" menuList=menuConfig.menuList currentUrl="/manage/profile/outbox"/>
...
<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/profile/getOutboxListForPage",
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','发件人','主题', '发送时间'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
                ...
                {name: 'userName', index: 'userName', width:200, fixed:true, align: 'left',sortable:true, resize:false},
                {name: 'title', index: 'title', align: 'left', sortable:true, resize:false,
                    formatter:function(value, grid, rows, state) {
                        return '<a href="#" onclick="viewOutbox('+rows.pkid+');return false;">'+rows.title+'</a>';
                    }
                },
        ...
        $("#btnDel").on("click", function () {
            layer.confirm("确定删除选中的已发送消息数据?", function (index) {
                layer.close(index);
                ...
                {
                    layer.msg("请选择要删除的已发送消息数据", {icon:2});
                    return;
                }
                jQuery.ajax({
                    type: "POST",
                    ...
                    dataType: "text",
                    url: "/manage/profile/doDelOutbox",
    ...
    {
        layer_show("浏览已发送消息内容", "/manage/profile/viewOubBox?id=" + id, 700, 500);
