                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <div style="margin-bottom: 5px;">
                                                <button id="btnAdd" style="float:left; margin-top: 2px" class="btn btn-success btn-mini radius-4"><i class="ace-icon fa fa-plus"></i>新消息</button>
                                                ...
                                                <label style="padding-top: 5px;" class="col-sm-1 no-padding-right" for="searchKey">主题搜索：</label>
                                                <input type="text" id="searchKey" style="width: 250px;" placeholder="输入消息主题" class="input-sm" />
...
<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/profile/getInboxListForPage",
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','','发件人','主题', '时间'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
                ...
                // align 为对齐方式；
                // sortable 表示是否可以排序
                {name: 'pkid', index: 'pkid',hidden:true, width:200, fixed:true, align: 'left',sortable:true, resize:false},
                {name: 'status', index: 'status',width:40, fixed:true, align: 'center', sortable:true, resize:false,
                    formatter:function(value, grid, rows, state) {
                        if(rows.status==1)
                        {
                            return '<i class="ace-icon fa fa-envelope-o bigger-120" title="已读"></i>';
                        }
                        else {
                            return '<i class="ace-icon fa fa-envelope bigger-120" title="未读"></i>';
                        }
                    }
                },
                {name: 'fromUserName', index: 'fromUserName', width:200, fixed:true, align: 'left',sortable:true, resize:false},
                {name: 'title', index: 'title', align: 'left', sortable:true, resize:false,
                    formatter:function(value, grid, rows, state) {
                        return '<a href="/manage/profile/viewMsg?id='+rows.pkid+'">'+rows.title+'</a>';
                    }
                },
        ...
        $("#btnAdd").on("click", function () {
            location.href = "/manage/profile/sendMsg";
        });
        ...
        $("#btnDel").on("click", function () {
            layer.confirm("确定删除选中的消息数据?", function (index) {
                layer.close(index);
                ...
                for(var i=0; i<ids.length; i++)
                {
                    ...
                    outboxIds.push(rowData.pkid);
                }
                if(outboxIds.length==0)
                {
                    layer.msg("请选择要删除的消息数据", {icon:2});
                    return;
                }
                jQuery.ajax({
                    type: "POST",
                    data: {"ids":outboxIds.join(",")},
                    dataType: "text",
                    url: "/manage/profile/doDelInbox",
