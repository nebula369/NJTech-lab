...
<div class="main-container ace-save-state" id="main-container">
    ...
    <!-- 加载左侧菜单 -->
    <@leftMenu indexUrl="/manage/eclassbrand/index" menuList=menuConfig.menuList currentUrl="/manage/eclassbrand/task"/>
                ...
                <div class="row">
                    <div class="col-xs-12" style="padding:0px;">
                        <div style="margin-bottom: 5px;">
                            <button id="btnAdd" style="float:left; margin-top: 2px"  class="btn btn-success btn-mini radius-4" value=""><i class="ace-icon fa fa-plus"></i>新增作业</button>
                            <label style="padding-top: 5px;" class="col-sm-0 no-padding-right" for="year"></label>
                            <input type="text" id="searchKey" style="width: 250px;" placeholder="请输入作业" class="input-sm" />
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            ...
            colModel: [
                ...
                {name: 'content', index: 'content', align: 'left',sortable:false, resize:false},
                {name: 'type', index: 'type', align: 'left',fixed:true, sortable:false, resize:false,
                    formatter:function(value, grid, rows, state){
                        if(rows.type==1) {return "家庭作业";  }
                        else {return "课后作业";}
                    }
                },
                {name: 'className', index: 'className', align: 'left',sortable:false, resize:false},
                {name: 'subjectName', index: 'subjectName', align: 'left',sortable:false, resize:false},
                {name: 'sortnum', index: 'sortnum', align: 'left',sortable:true, resize:false},
                {
                    ...
                    formatter: function (value, grid, rows, state) {
                        ...
                        html = "<button class='btn btn-minier btn-success' onclick='modify("+rows.pkid+");'><i class=\"ace-icon fa fa-pencil\"></i>编辑</button>&nbsp;&nbsp;" +
                            "<button class='btn btn-minier btn-danger' onclick='del("+rows.pkid+",\""+rows.content+"\");'><i class=\"ace-icon fa fa-trash-o\"></i>删除</button>";
                        return html;
                    }
                }
            ],
        ...
        $("#btnAdd").on("click", function () {
            layer_show("新建作业", "/manage/eclassbrand/task/add", 550, 440);
    ...
    function setUser(id) {
        layer_show("设置班级", "/manage/eclassbrand/task/relationuser?id="+id, 800, 600,setCurrentLayerObj);
        event.stopPropagation();
    }
    function reloadData()
    {
    ...
    function modify(id)
    {
        layer_show("编辑作业", "/manage/eclassbrand/task/edit?id=" + id, 550, 440);
        event.stopPropagation();
    }
    ...
    function del(id, name)
    {
        ...
        layer.confirm("确定删除作业“"+name+"”？", function (index) {
            layer.close(index);
            jQuery.ajax({
                type: "POST",
                data: {"id":id},
                dataType: "text",
                url: "/manage/eclassbrand/task/doDel",
