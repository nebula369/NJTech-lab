...
<div class="main-container ace-save-state" id="main-container">
    ...
    <!-- 加载左侧菜单 -->
    <@leftMenu indexUrl="/manage/eclassbrand/index" menuList=menuConfig.menuList currentUrl="/manage/eclassbrand/recommend"/>
                ...
                <div class="row">
                    <div class="col-xs-12" style="padding:0px;">
                        <div style="margin-bottom: 5px;">
                            <button id="btnAdd" style="float:left; margin-top: 2px" class="btn btn-success btn-mini radius-4" value=""><i class="ace-icon fa fa-plus"></i>新增推荐</button>
                            <label style="padding-top: 5px;" class="col-sm-0 no-padding-right" for="year"></label>
                            <input type="text" id="searchKey" style="width: 250px;padding-left: 20px" placeholder="请输入推荐名称" class="input-sm" />
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/eclassbrand/recommend/getRecommendListForPage",
            postData: {unitId: $("#unitId").val(),searchKey:$("#searchKey").val()},
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','推荐名称','班级名称','排序', '操作'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
        ...
        $("#btnAdd").on("click", function () {
            layer_show("新建推荐", "/manage/eclassbrand/recommend/add", 1200, 860);
    ...
    function modify(id)
    {
        layer_show("编辑推荐", "/manage/eclassbrand/recommend/edit?id=" + id, 1200, 860);
        event.stopPropagation();
    }
    ...
    function del(id, name)
    {
        ...
        layer.confirm("确定删除推荐“"+name+"”？", function (index) {
            layer.close(index);
            jQuery.ajax({
                type: "POST",
                data: {"id":id},
                dataType: "text",
                url: "/manage/eclassbrand/recommend/doDel",
