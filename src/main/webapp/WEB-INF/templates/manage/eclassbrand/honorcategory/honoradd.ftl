...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $("#validation-form .btn-success").on("click", function () {
            ...
            jQuery.ajax({
                type: "POST",
                data: {"categoryid":${categoryid},"name":$("#name").val(),"sortnum":$("#sortnum").val(),"picture":$("#picture").val()},
                dataType: "text",
                url: "/manage/eclassbrand/honorcategory/honordoAdd",
                success: function (result) {
                    if(result == 1) {
                        layer.msg("保存成功", {icon: 1, time: 1000}, function () {
                            parent.honorListReloadData();
                            layer_close();
                        });
                    }
                    else
