...
<div class="main-container ace-save-state" id="main-container">
    ...
    <!-- 加载左侧菜单 -->
    <@leftMenu indexUrl="/manage/eclassbrand/index" menuList=menuConfig.menuList currentUrl="/manage/eclassbrand/honorcategory"/>
                ...
                <div class="row">
                    <div class="col-xs-12" style="padding:0px;">
                        <div style="margin-bottom: 5px;">
                            <button id="btnAdd" style="float:left; margin-top: 2px" class="btn btn-success btn-mini radius-4" value=""><i class="ace-icon fa fa-plus"></i>新增荣誉分类</button>
                            <label style="padding-top: 5px;" class="col-sm-0 no-padding-right" for="year"></label>
                            <input type="text" id="searchKey" style="width: 250px;margin-left: 20px" placeholder="请输入荣誉分类" class="input-sm" />
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/eclassbrand/honorcategory/getHonorCategoryListForPage",
            postData: {categoryid: $("#categoryid").val(),searchKey:$("#searchKey").val()},
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','荣誉分类名称','班级名称','排序', '操作'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
                ...
                {name: 'className', index: 'className', align: 'left',sortable:true, resize:false},
                {name: 'sortnum', index: 'sortnum', align: 'left',sortable:true, resize:false},
                {
                    ...
                    formatter: function (value, grid, rows, state) {
                        ...
                        html = "<button class='btn btn-minier btn-success' onclick='modify("+rows.pkid+");'><i class=\"ace-icon fa fa-pencil\"></i>编辑</button>&nbsp;&nbsp;" +
                            ...
                            "<button class='btn btn-minier btn-info' onclick='sethonor("+rows.pkid+");'><i class=\"ace-icon fa fa-check-square-o\"></i>设置荣誉</button>";
                        return html;
                    }
                }
            ],
        ...
        $("#btnAdd").on("click", function () {
            layer_show("新建荣誉分类", "/manage/eclassbrand/honorcategory/add", 530, 280);
    ...
    function modify(id)
    {
        layer_show("编辑荣誉分类", "/manage/eclassbrand/honorcategory/edit?id=" + id, 530, 280);
        event.stopPropagation();
    }
    ...
    {
        layer_show("设置荣誉", "/manage/eclassbrand/honorcategory/honorlist?id=" + id, 1200, 800,setCurrentLayerObj);
        event.stopPropagation();
    }
    ...
    function del(id, name)
    {
        ...
        layer.confirm("确定删除荣誉分类“"+name+"”？", function (index) {
            layer.close(index);
            jQuery.ajax({
                type: "POST",
                data: {"id":id},
                dataType: "text",
                url: "/manage/eclassbrand/honorcategory/doDel",
