                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <div style="margin-bottom: 5px;">
                                                <button id="btnAdd" style="float:left; margin-top: 5px" class="btn btn-success btn-mini radius-4"><i class="ace-icon fa fa-plus"></i>选择用户</button>
                                                ...
                                                <input type="text" id="searchKey" style="width: 250px;" placeholder="请输入考生姓名" class="input-sm" />
...
<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/eclassbrand/examplan/getUserListForPage",
            postData: {examPlanId: ${examplanId},searchKey:$("#searchKey").val()},
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','照片','用户姓名','来源', '准考证号', '学生编号', '操作'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
                ...
                {name: 'photo', index: 'photo', align: 'left', sortable:true, resize:false,
                    formatter:function(value, grid, rows, state){
                        return "<span>  <img src=\"/"+rows.photo+"\"  id=\"headimg\" alt=\"头像\" width=\"30px\" height=\"60px\"   onerror=\"javascript:this.src='/assets/lib/images/avatars/avatar2.png';\"></span>";
                    }
                },
                {name: 'name', index: 'name', align: 'left',sortable:false, resize:false},
                {name: 'type', index: 'type', align: 'left', sortable:true, resize:false,
                    formatter:function(value, grid, rows, state){
                        if(rows.type==1)
                        {
                        ...
                        {
                            return "<span>第三方导入</span>";
                        }
                    }
                },
                {name: 'number', index: 'number', align: 'left',sortable:true, resize:false},
                {name: 'studentid', index: 'studentid', align: 'left',sortable:true, resize:false},
                {
                    name: 'action', index: '', sortable: false,fixed:true, width:100, resize: false,
                    formatter: function (value, grid, rows, state) {
                        var html = "<button class='btn btn-minier btn-danger' title='删除' onclick='del("+rows.pkid+",\""+rows.name+"\");'><i class=\"ace-icon fa fa-trash-o\"></i>移除</button>";
                        return html;
                    }
                }
            ],
        ...
        $("#btnDel").on("click", function () {
            layer.confirm("确定删除选中的考生?", function (index) {
                layer.close(index);
                ...
                {
                    layer.msg("请选择要删除的考生", {icon:2});
                    return;
                }
                jQuery.ajax({
                    type: "POST",
                    data: {"ids":teacherIds.join(","),"examPlanId":${examplanId}},
                    dataType: "text",
                    url: "/manage/eclassbrand/examplan/DoExamPlanMineeDel",
    ...
    function initDataList()
    {
        ...
        var postJson = {examplanId: ${examplanId}, searchKey:searchKey};
        //传入查询条件参数
    ...
    {
        layer.confirm("确定移除用户“"+name+"”？", function (index) {
            layer.close(index);
            jQuery.ajax({
                type: "POST",
                data: {"ids":id,"examPlanId":${examplanId}},
                dataType: "text",
                url: "/manage/eclassbrand/examplan/DoExamPlanMineeDel",
                success: function (result) {
                    layer.msg("删除成功", {icon: 1, time: 1000}, function () {
    ...
    function setSelectedUser(selectUsers)
    {
        ...
        jQuery.ajax({
            type: "POST",
            data: {"ids":selectRoleUserIds.join(","),"examPlanId":${examplanId}},
            dataType: "text",
            url: "/manage/eclassbrand/examplan/DoExamPlanMineeAdd",
            success: function (result) {
