...
<div class="main-container ace-save-state" id="main-container">
    ...
    <!-- 加载左侧菜单 -->
    <@leftMenu indexUrl="/manage/eclassbrand/index" menuList=menuConfig.menuList currentUrl="/manage/eclassbrand/news"/>
                ...
                <div class="row">
                    <div class="col-xs-12" style="padding:0px;">
                        <div style="margin-bottom: 5px;">
                            <button id="btnAdd" style="float:left; margin-top: 2px" class="btn btn-success btn-mini radius-4" value=""><i class="ace-icon fa fa-plus"></i>新增新闻分类</button>
                            <label style="padding-top:2px;" class="col-sm-0 no-padding-right" for="year"></label>
                            <input type="text" id="searchKey" style="width: 250px;" placeholder="请输入新闻分类" class="input-sm" />
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/eclassbrand/news/getNewsTypeListForPage",
            postData: {newtypeid: $("#categoryid").val(),searchKey:$("#searchKey").val()},
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','新闻分类名称','班级名称','操作'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
                ...
                {name: 'className', index: 'className', align: 'left',sortable:true, resize:false},
                {
                    ...
                    formatter: function (value, grid, rows, state) {
                        ...
                        html = "<button class='btn btn-minier btn-success' onclick='modify("+rows.pkid+");'><i class=\"ace-icon fa fa-pencil\"></i>编辑</button>&nbsp;&nbsp;" +
                            ...
                            "<button class='btn btn-minier btn-info' onclick='sethonor("+rows.pkid+");'><i class=\"ace-icon fa fa-check-square-o\"></i>设置新闻</button>";
                        return html;
                    }
                }
            ],
        ...
        $("#btnAdd").on("click", function () {
            layer_show("新建新闻分类", "/manage/eclassbrand/news/typeadd", 530, 280);
    ...
    function modify(id)
    {
        layer_show("编辑新闻分类", "/manage/eclassbrand/news/typeedit?id=" + id, 530, 280);
        event.stopPropagation();
    }
    ...
    {
        layer_show("设置新闻", "/manage/eclassbrand/news/newslist?newstypeid=" + id, window.innerWidth*0.9, window.innerHeight*0.95,setCurrentLayerObj);
        event.stopPropagation();
    }
    ...
    function del(id, name)
    {
        ...
        layer.confirm("确定删除新闻分类“"+name+"”？", function (index) {
            layer.close(index);
            jQuery.ajax({
                type: "POST",
                data: {"id":id},
                dataType: "text",
                url: "/manage/eclassbrand/news/doTypeDel",
