                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <div style="margin-bottom: 5px;">
                                                ...
                                                <input type="text" id="searchKey" style="width: 250px;" placeholder="请输入新闻名称" class="input-sm" />
...
<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        $("#grid-table").jqGrid({
            url: "/manage/eclassbrand/news/getNewsListForPage",
            postData: {newstypeid: ${newstypeid},searchKey:$("#searchKey").val()},
            datatype: "json", // 从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
            mtype: "POST", // 提交方式，默认为GET
            height: "auto", // 高度，表格高度。可为数值、百分比或'auto'
            colNames: ['编号','新闻名称','作者', '操作'], // 列显示名称，是一个数组对象
            colModel: [
                // name 表示列显示的名称；
                ...
                {name: 'author', index: 'author', align: 'left',sortable:true, resize:false},
                {
        ...
        $("#btnAdd").on("click", function () {
            layer_show("添加新闻", "/manage/eclassbrand/news/newsadd?newstypeid="+${newstypeid}, 1200, 817);
        ...
        $("#btnDel").on("click", function () {
            layer.confirm("确定删除选中的新闻?", function (index) {
                layer.close(index);
                ...
                jQuery.ajax({
                    ...
                    data: {"ids":Ids.join(",")},
                    dataType: "text",
                    url:  "/manage/eclassbrand/news/doDel",
   ...
   /* function honorListReloadData()
    ...
    function initDataList()
    {
        ...
        var postJson = {newstypeid: ${newstypeid}, searchKey:searchKey};
        //传入查询条件参数
        $("#grid-table").jqGrid("setGridParam",{postData:postJson, page:1}).trigger("reloadGrid");
    }
    function modify(id) {
        layer_show("编辑新闻", "/manage/eclassbrand/news/newsedit?id=" + id + "&newstypeid="+${newstypeid}, 1200, 817);
        event.stopPropagation();
    }

    function del(id, name)
    {
        layer.confirm("确定删除新闻“"+name+"”？", function (index) {
            layer.close(index);
            jQuery.ajax({
                type: "POST",
